'''
    功能：tushare表导入mysql
    描述：日级交易数据（非复权）+ 复权因子
    刷数：长周期:设置[start_date, end_date] (单code请求)
         短周期: 按天刷(批量code请求)
'''
...
def request_from_tushare(ts_codes):
    ...
    if start_date == end_date:
        ...
        for i, code in enumerate(ts_codes):
            time.sleep(0.06)  # API调用频次：1min不超过1000次。60/1000=0.06
            if (i + 1) % 100 == 0:
...
def write_to_mysql(data):
    ...
    with conn.cursor() as cursor:
        ...
        sql = '''
                    INSERT INTO trade_daily_ts({}) VALUES ({})
                '''.format(','.join(fea_merge).replace('change', '`change`'), ','.join(fea_merge_format))
...
def get_stocks():
    ...
    if df.empty or len(df) == 0:
        raise Exception('table stock_info_ts has no data: {}'.format(today))
...
def main():
    try:
        ...
        send_email('Data: trade_daily_ts', error_info)
