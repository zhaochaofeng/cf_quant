...
class DockerEnv(Env[DockerConf]):
    ...
    def prepare(self, *args, **kwargs) -> None:  # type: ignore[no-untyped-def]
        """
        Download or build Docker image if it doesn't exist.

        This method:
        1. Checks if the image already exists locally
        2. Builds from Dockerfile if needed and configured
        3. Pulls from registry if image still doesn't exist
        """
        client = docker.from_env()
        ...
        try:
            client.images.get(self.conf.image)
            image_exists = True
            logger.info(f"Image {self.conf.image} already exists locally, skipping build/pull.")
        except docker.errors.ImageNotFound:
            logger.info(f"Image {self.conf.image} not found locally.")

        # Only build if image doesn't exist and build_from_dockerfile is enabled
        if (
                not image_exists
                and self.conf.build_from_dockerfile
                and self.conf.dockerfile_folder_path is not None
                and self.conf.dockerfile_folder_path.exists()
        ):
            logger.info(f"Building the image from dockerfile: {self.conf.dockerfile_folder_path}")
                    ...
                    for line in lines:
                        if line.strip():
                            ...
                            if "stream" in status_dict:
                                p.update(task, description=status_dict["stream"])
            logger.info(f"Finished building the image from dockerfile: {self.conf.dockerfile_folder_path}")
            image_exists = True  # Mark image as existing after successful build

        # Only pull if image still doesn't exist (wasn't built from Dockerfile)
        if not image_exists:
            logger.info(f"Pulling image {self.conf.image} from registry...")
            try:
                image_pull = client.api.pull(self.conf.image, stream=True, decode=True)
                current_status = ""
                layer_set = set()
