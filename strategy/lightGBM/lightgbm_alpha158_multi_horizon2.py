...
class LightGBMAlpha158:
    def __init__(self,
        ...
        self.features_learn = None
        self.features_infer = None
    ...
    def main(self):
        ...
        self.features_learn = pd.concat(dataset.prepare(segments=['train', 'valid', 'test'], col_set=['feature'], data_key='learn'), axis=0)
        self.features_infer = pd.concat(dataset.prepare(segments=['train', 'valid', 'test'], col_set=['feature'], data_key='infer'), axis=0)

        mh_gen = Alpha158MultiHorizonGen(horizon=self.horizon, label_leak_n=2)
        tasks = mh_gen.generate(task)   # 任务配置集合
        ...
        for i, hr in enumerate(self.horizon):
            ...
            label_learn, label_infer = self.get_label(
                                    task['dataset']['kwargs']['handler']['kwargs']['start_time'],
                                    task['dataset']['kwargs']['handler']['kwargs']['end_time'],
                                    hr, instruments)

            self.features_learn[label_learn.columns] = label_learn
            self.features_learn.dropna(inplace=True, axis=0, subset=label_learn.columns)
            self.features_infer[label_infer.columns] = label_infer
            ...
            dataset_learn = self.dataframe_to_dataseth(self.features_learn, segs)
            dataset_infer = self.dataframe_to_dataseth(self.features_infer, segs)

            print('dataset_learn: {}'.format('-' * 100))
            print("features_learn shape: {}".format(self.features_learn.shape))
            ...
            # 
            # 说明：Python 的引用计数机制
            # 1. del 只删除变量名，如果对象还有其他引用，对象不会被释放
            # 2. 引用链：par → port_analysis_config → dataset_infer 对象
            #            sr → dataset_infer 对象
            # 3. 如果只 del dataset_infer，但 port_analysis_config 和 sr 仍引用它，对象不会被释放
            # 4. 因此需要按引用链顺序删除：先删除持有引用的对象（par, sr），再删除被引用的对象
