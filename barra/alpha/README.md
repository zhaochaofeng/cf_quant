...
12. **与 APM 记法的 K 差异**：APM §11A 中 $K$ 为总预测信号元素数（$K = J \cdot N$，J 为每资产信号数、N 为资产数），$\mathbf{g}$ 为 $JN$ 维向量、$\operatorname{Var}\{\mathbf{g}\}$ 与 $H$ 均为 $JN \times JN$ 方阵（"两个预测"节即 $K=2N$）。本 README 的 $K$ 是**每只资产的信号数**（对应 APM 的 $J$），$\Sigma_z$ 与 $H$ 为 $K \times K$。差异源于工程实现用逐日横截面 z-score（4.2）消化资产维，$\Sigma_z$（5.2）只保留信号间相关结构，参数从 $O((JN)^2)$ 降至 $O(K^2)$；正交化公式（11A-10~13）同构，仅作用空间由 $JN$ 压缩至 $K$。