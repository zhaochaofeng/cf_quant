...
class QPOptimizer:
    ...
    def solve(
        ...
        # 目标函数: min 0.5 * h' * Q * h - α'h + c_b'b + c_s's
        # 其中 Q = 2λV
        ...
        try:
            ...
            logger.warning(f'OSQP求解失败，尝试ECOS: {e}')
            try:
                problem.solve(solver=cp.ECOS, verbose=False)
            except Exception as e2:
                logger.error(f'ECOS求解也失败: {e2}')
    ...
    def _ensure_positive_definite(self, V: np.ndarray) -> np.ndarray:
        ...
        if min_eigenvalue < 1e-10:
            # 添加小正数使其正定
            logger.warning(f'协方差矩阵非正定，最小特征值={min_eigenvalue:.2e}，进行修正')
            eigenvalues = np.maximum(eigenvalues, 1e-10)
            V = eigenvectors @ np.diag(eigenvalues) @ eigenvectors.T
